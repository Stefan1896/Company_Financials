...
ggplot(financials, aes(sector)) +
  geom_bar(fill = "indianred", alpha = 0.2, color = "black") +
  theme_classic() +
...
#loop through numeric variable names to plot each numeric variable in its natural form with sector
#pn <- list()
#j = 1
#for(i in c(nums_names)){
#  pn[[j]] <- financials %>% filter(sector != "Telecommunication Services") %>% 
#                  ggplot(aes(x=sector, y = !!sym(i))) +
#                  geom_boxplot(outlier.shape = NA) + 
#                  geom_jitter(width=0.1, alpha=0.1, color = "indianred") +
#                  labs(x = "", y = nums_original_names[j]) +
#                  theme_classic() +
#                  coord_flip()
#  j = j+1
#}
...
for(i in c(nums_names)){
  pt[[j]] <- financials %>% filter(sector != "Telecommunication Services") %>% 
                  ...
                  geom_boxplot(outlier.shape = NA) + 
                  geom_jitter(width=0.1, alpha=0.2, color = "indianred") +
...
for(i in c(selected_financials[selected_financials != "price_earnings"])){
  ps[[j]] <- financials %>% 
                  ...
                  geom_smooth(method='lm', formula= y~x, color = "indianred") +
                  labs(x = "", y = "") +
...
ps <- list()
j = 1
for(i in c(selected_financials[selected_financials != "price_bok"])){
  ps[[j]] <- financials %>% 
